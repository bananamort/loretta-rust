...
- `errors/luaresources.rs` — `LuaResources.resx` + `.Designer.cs` as one resource module.
- `symbol_display/unicode_categories.rs` — generated .NET `CharUnicodeInfo` oracle table.
- `experimental/syntaxextensions.rs` — DROP tombstone preserving the obsolete C#
  `SyntaxExtensions.FoldConstants` verbatim for the audit trail.
