...
static async Task<object> RunOperation(string operation, LuaParseOptions parseOpts, string code, string label)
{
    ...
    return operation switch
    {
        "options" => new
        {
            ...
            withDocumentationMode = parseOpts.WithDocumentationMode(Loretta.CodeAnalysis.DocumentationMode.None).DocumentationMode.ToString(),
            ...
            equalsObjectDefault = parseOpts.Equals((object) new LuaParseOptions(syntaxOpts)),
...
static object CharUtilsOp(string code)
{
    ...
    var isAlphaNumeric = charUtilsType.GetMethod("IsAlphaNumeric", System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Static)!;
    var isHexadecimal = charUtilsType.GetMethod("IsHexadecimal", System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Static)!;
    var isValidTrailingIdentifierChar = charUtilsType.GetMethod("IsValidTrailingIdentifierChar", System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Static)!;
    return new { results = code.EnumerateRunes().Select(r => new { ch = r.ToString(), isBinary = r.Value <= char.MaxValue && (bool) isBinary.Invoke(null, new object[] { (char) r.Value })!, isDecimal = r.Value <= char.MaxValue && (bool) isDecimal.Invoke(null, new object[] { (char) r.Value })!, isOctal = r.Value <= char.MaxValue && (bool) isOctal.Invoke(null, new object[] { (char) r.Value })!, isWhitespace = r.Value <= char.MaxValue && (bool) isWhitespace.Invoke(null, new object[] { (char) r.Value })!, isValidFirstIdentifierChar = r.Value <= char.MaxValue && (bool) isValidFirstIdentifierChar.Invoke(null, new object[] { (char) r.Value })!, isAlphaNumeric = r.Value <= char.MaxValue && (bool) isAlphaNumeric.Invoke(null, new object[] { (char) r.Value })!, isHexadecimal = r.Value <= char.MaxValue && (bool) isHexadecimal.Invoke(null, new object[] { (char) r.Value })!, isValidTrailingIdentifierChar = r.Value <= char.MaxValue && (bool) isValidTrailingIdentifierChar.Invoke(null, new object[] { (char) r.Value })! }).ToArray() };
}
