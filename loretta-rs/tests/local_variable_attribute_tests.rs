...
// values. The two error tests assert the full_moon parse errors are
// non-empty (the C# codes LUA1001/LUA1006 differ from the full_moon error
// messages).
...
fn parser_generates_an_error_diagnostic_when_identifier_is_missing() {
    ...
    // reports the missing attribute identifier ("expected identifier after
    // `<` for attribute"); the port asserts the errors are non-empty.
    let result = full_moon::parse_fallible("local a <>", full_moon::LuaVersion::new());
    assert!(
        !result.errors().is_empty(),
        "the missing attribute identifier must error"
    );
}

#[test]
fn parser_generates_an_error_diagnostic_when_closing_token_is_missing() {
    // The C# expects ERR_SyntaxError ("> expected") — the full_moon reports
    // "expected `>` to close attribute"; the port asserts the errors are
    // non-empty.
    let result = full_moon::parse_fallible("local a<const", full_moon::LuaVersion::new());
    assert!(
        !result.errors().is_empty(),
        "the missing closing token must error"
    );
}
